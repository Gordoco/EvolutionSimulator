...
public class GenerateTerrain : MonoBehaviour
{
    ...
    private Vector3[][] sidedVertices; //Copied from verticies
    private int[][] sidedIndexs; //Matched to indecies in verticies
    /* * * * * * * * * * * * */
    ...
    void CreateVerts() {
        ...
        for (int i = 0; i <= partitions; i++) {
            for (int j = 0; j <= partitions; j++) {
                ...
                float xParam = (((xSize / partitions) * i) + gameObject.transform.position.x + 10.5f) / scale;
                float yParam = (((zSize / partitions) * j) + gameObject.transform.position.z + 10.5f) / scale;

                float y = Mathf.PerlinNoise(xParam, yParam) * severity;
