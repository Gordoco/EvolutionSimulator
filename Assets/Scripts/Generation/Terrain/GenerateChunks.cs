...
public class GenerateChunks : MonoBehaviour
{
    ...
    void initGenerateNewChunks() {
        ...
        if (!followPlayer) createBorder((2 * viewDist) + 3, chunkSize);
    }
    ...
    GameObject createChunk(int i, int j, int x) {
        ...
        //GameObject newSection = Instantiate(terrainPrefab, new Vector3(i*chunkSize, 0, j*chunkSize), Quaternion.identity) as GameObject;
    ...
    void createBorder(int width, int chunkSize)
    {
        GenerateBorder BorderGenerator = gameObject.AddComponent(typeof(GenerateBorder)) as GenerateBorder;
        BorderGenerator.initialize(width, chunkSize, borderObject);
    }
    ...
    void updateChunks(Direction dir) {
        ...
        switch(dir) {
            case Direction.RIGHT:
                ...
                for (int i = dist * (dist-1); i < dist * dist; i++) //CREATE ON LEFT
                {
